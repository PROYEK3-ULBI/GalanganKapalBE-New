...
// Register attaches vendor endpoints under r.
//   - protectedMW: any authenticated user can read.
//   - mutateMW: only roles allowed to mutate (admin) can create/update/delete.
func (h *Handler) Register(r fiber.Router, protectedMW fiber.Handler, mutateMW fiber.Handler) {
	g := r.Group("/vendors", protectedMW)
	g.Get("/", h.list)
	g.Get("/:id", h.get)
...
func (h *Handler) get(c *fiber.Ctx) error {
	v, err := h.svc.Get(c.UserContext(), c.Params("id"))
	if err != nil {
		return mapError(err)
	}
	return c.JSON(v)
